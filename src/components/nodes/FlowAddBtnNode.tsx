...
function BranchDropdown({
  ...
  return (
    <div
      ref={ref}
      className="nodrag nopan absolute left-1/2 -translate-x-1/2"
    ...
    >
      ...
      >
        <button
          ...
          onClick={(e) => { e.stopPropagation(); onSelect("segmentar"); }}
        >
        ...
        <button
          className="w-full flex items-center gap-3 px-3 py-[11px] hover:bg-[#F5F6F7] transition-colors text-left"
          onClick={(e) => { e.stopPropagation(); onSelect("nao-atende"); }}
        >
...
export default function FlowAddBtnNode({ data }: NodeProps) {
  ...
  return (
    <div className="nodrag nopan relative">
      {/* Linha pontilhada vertical acima do botão (só para "Adicionar ramificação") */}
      {d._placement === "below" && (
        <div
          style={{
            position: "absolute",
      ...
      {dropOpen && d.isAddBranch && (
        <BranchDropdown onSelect={handleSelect} onClose={handleClose} />
      )}
    </div>
  );
}
